...
负责将生成的 interaction.md、persona.md 写入到正确的目录结构，
并生成 meta.json 和完整的 SKILL.md。

用法：
    python3 skill_writer.py --action create --slug zhang-shifu --meta meta.json \
        --interaction interaction_content.md --persona persona_content.md \
        --base-dir ./father-in-laws

    python3 skill_writer.py --action update --slug zhang-shifu \
        --interaction-patch interaction_patch.md --persona-patch persona_patch.md \
        --base-dir ./father-in-laws

    python3 skill_writer.py --action list --base-dir ./father-in-laws
"""
...
def slugify(name: str) -> str:
    ...
    return slug if slug else "father_in_law"
...
def build_identity_string(meta: dict) -> str:
    ...
    father_in_law_type = profile.get("father_in_law_type", "")
    ...
    identity = "".join(parts) if parts else "老丈人"

    if father_in_law_type:
        identity += f"，{father_in_law_type}"
...
def create_skill(
    ...
    # 写入 interaction.md
    (skill_dir / "interaction.md").write_text(interaction_content, encoding="utf-8")
    ...
    # 写入 interaction-only skill
    interaction_only = (
        f"---\nname: father_in_law_{slug}_interaction\n"
        f"description: {name} 的关系模式（仅 Interaction，无 Persona）\n"
        f"user-invocable: true\n---\n\n{interaction_content}\n"
    )
    (skill_dir / "interaction_skill.md").write_text(interaction_only, encoding="utf-8")

    # 写入 persona-only skill
    persona_only = (
        f"---\nname: father_in_law_{slug}_persona\n"
        f"description: {name} 的人物性格（仅 Persona，无关系模式）\n"
        f"user-invocable: true\n---\n\n{persona_content}\n"
    )
...
def update_skill(
    skill_dir: Path,
    interaction_patch: Optional[str] = None,
    ...
    for fname in ("SKILL.md", "interaction.md", "persona.md"):
        src = skill_dir / fname
        if src.exists():
            shutil.copy2(src, version_dir / fname)

    # 应用 interaction patch
    if interaction_patch:
        current_interaction = (skill_dir / "interaction.md").read_text(encoding="utf-8")
        new_interaction = current_interaction + "\n\n" + interaction_patch
        (skill_dir / "interaction.md").write_text(new_interaction, encoding="utf-8")
    ...
    # 重新生成 SKILL.md
    interaction_content = (skill_dir / "interaction.md").read_text(encoding="utf-8")
...
def list_father_in_laws(base_dir: Path) -> list:
    """列出所有已创建的老丈人 Skill"""
    father_in_laws = []

    if not base_dir.exists():
        return father_in_laws
...
def main() -> None:
    parser = argparse.ArgumentParser(description="老丈人 Skill 文件写入器")
    parser.add_argument("--action", required=True, choices=["create", "update", "list"])
    parser.add_argument("--slug", help="老丈人 slug（用于目录名）")
    parser.add_argument("--name", help="老丈人称呼")
    parser.add_argument("--meta", help="meta.json 文件路径")
    parser.add_argument("--interaction", help="interaction.md 内容文件路径")
    parser.add_argument("--persona", help="persona.md 内容文件路径")
    parser.add_argument("--interaction-patch", help="interaction.md 增量更新内容文件路径")
    parser.add_argument("--persona-patch", help="persona.md 增量更新内容文件路径")
    parser.add_argument(
        "--base-dir",
        default="./father-in-laws",
        help="老丈人 Skill 根目录（默认：./father-in-laws）",
    )
    ...
    if args.action == "list":
        father_in_laws = list_father_in_laws(base_dir)
        if not father_in_laws:
            print("暂无已创建的老丈人 Skill")
        else:
            print(f"已创建 {len(father_in_laws)} 个老丈人 Skill：\n")
            for c in father_in_laws:
        ...
        slug = args.slug or slugify(meta.get("name", "father_in_law"))

        interaction_content = ""
        if args.interaction:
            interaction_content = Path(args.interaction).read_text(encoding="utf-8")

        persona_content = ""
        if args.persona:
            persona_content = Path(args.persona).read_text(encoding="utf-8")

        skill_dir = create_skill(base_dir, slug, meta, interaction_content, persona_content)
        print(f"✅ 老丈人 Skill 已创建：{skill_dir}")
        print(f"   触发词：/{slug}")
        ...
        interaction_patch = Path(args.interaction_patch).read_text(encoding="utf-8") if args.interaction_patch else None
        persona_patch = Path(args.persona_patch).read_text(encoding="utf-8") if args.persona_patch else None

        new_version = update_skill(skill_dir, interaction_patch, persona_patch)
        print(f"✅ 老丈人 Skill 已更新到 {new_version}：{skill_dir}")
